			     ...
			     struct ad7124_st_reg* p_reg)
{
	...
	if (p_reg->addr != AD7124_ERR_REG && p_reg->addr != AD7124_DATA_REG && dev->check_ready) {
				  ...
				  uint32_t timeout)
{
	...
	gpio_set_irq_enabled(d_out_rdy_pin, GPIO_IRQ_EDGE_FALL, false);
 ...
 * @return Returns 0 for success or negative error code.
*******************************************************************************/
uint32_t ad7124_setup(struct ad7124_dev **device,
		     struct ad7124_init_param init_param)
{
	...
	if (!dev)
		return INVALID_VAL;
	dev->ad7124_xArrayIndex = init_param.ad7124_xArrayIndex;
